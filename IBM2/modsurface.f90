...
!> Calculates the interaction with the soil, the surface temperature and humidity, and finally the surface fluxes.
  subroutine surface
    ...
    integer  :: i, j, n, patchx, patchy,k !step IBM: k for applying surface parameterization at the height of the ground
    ...
    if(lhetero) then
      ...
      do j = 2, j1
        do i = 2, i1
          ...
          patchx = patchxnr(i)
          patchy = patchynr(j)
          upatch(patchx,patchy) = upatch(patchx,patchy) + 0.5 * (u0(i,j,1) + u0(i+1,j,k+1)) !cstep IBM
          vpatch(patchx,patchy) = vpatch(patchx,patchy) + 0.5 * (v0(i,j,1) + v0(i,j+1,k+1)) !cstep IBM
    ...
    if(isurf <= 2) then
      ...
      do j = 2, j1
        do i = 2, i1
          ...
          Cm(i,j) = fkar ** 2. / (log(zf(k+1) / z0m(i,j)) - psim(zf(k+1) / obl(i,j)) + psim(z0m(i,j) / obl(i,j))) ** 2.
          Cs(i,j) = fkar ** 2. / (log(zf(k+1) / z0m(i,j)) - psim(zf(k+1) / obl(i,j)) + psim(z0m(i,j) / obl(i,j))) / &
          (log(zf(k+1) / z0h(i,j)) - psih(zf(k+1) / obl(i,j)) + psih(z0h(i,j) / obl(i,j)))

          if(lmostlocal) then
            upcu  = 0.5 * (u0(i,j,k+1) + u0(i+1,j,k+1)) + cu !cstep IBM
            vpcv  = 0.5 * (v0(i,j,k+1) + v0(i,j+1,k+1)) + cv !cstep IBM
    ...
    if(isurf <= 2) then
      do j = 2, j1
        do i = 2, i1
          ...
          horv   = sqrt(upcu ** 2. + vpcv ** 2.)
          horv   = max(horv, 0.1)
          horvav = sqrt(u0av(k+1) ** 2. + v0av(k+1) ** 2.) !cstep IBM, take care, slab mean goes through buildings
          ...
          thlflux(i,j) = - ( thl0(i,j,k+1) - tskin(i,j) ) / ra(i,j)   !cstep IBM
          qtflux(i,j) = - (qt0(i,j,k+1)  - qskin(i,j)) / ra(i,j)      !cstep IBM
          ...
          phimzf = phim(zf(k+1)/obl(i,j))
          phihzf = phih(zf(k+1)/obl(i,j))
          
          dudz  (i,j) = ustar(i,j) * phimzf / (fkar*zf(k+1))*(upcu/horv)
          dvdz  (i,j) = ustar(i,j) * phimzf / (fkar*zf(k+1))*(vpcv/horv)
          dthldz(i,j) = - thlflux(i,j) / ustar(i,j) * phihzf / (fkar*zf(k+1))
          dqtdz (i,j) = - qtflux(i,j)  / ustar(i,j) * phihzf / (fkar*zf(k+1))
        end do
      end do
      ...
      if(lsmoothflux) then
        ...
        do j = 2, j1
          do i = 2, i1
            ...
            dudz  (i,j) = ustar(i,j) * phimzf / (fkar*zf(k+1))*(upcu/horv) !cstep IBM 
            dvdz  (i,j) = ustar(i,j) * phimzf / (fkar*zf(k+1))*(vpcv/horv) !cstep IBM
            dthldz(i,j) = - thlflux(i,j) / ustar(i,j) * phihzf / (fkar*zf(k+1)) !cstep IBM
            dqtdz (i,j) = - qtflux(i,j)  / ustar(i,j) * phihzf / (fkar*zf(k+1)) !cstep IBM
          end do
        end do
      ...
      do j = 2, j1
        do i = 2, i1
          ...
          horv   = sqrt(upcu ** 2. + vpcv ** 2.)
          horv   = max(horv, 0.1)
          horvav = sqrt(u0av(k+1) ** 2. + v0av(k+1) ** 2.) !cstep IBM
          horvav = max(horvav, 0.1)
          if( isurf == 4) then !cstep IBM
            if(lmostlocal) then
              ustar (i,j) = fkar * horv  / (log(zf(k+1) / z0m(i,j)) - psim(zf(k+1) / obl(i,j)) + psim(z0m(i,j) / obl(i,j)))
            else
              if(lhetero) then
                ustar (i,j) = fkar * horvpatch(patchx,patchy) / (log(zf(k+1) / z0m(i,j)) - psim(zf(k+1) / obl(i,j))&
                + psim(z0m(i,j) / obl(i,j)))
              else
                ustar (i,j) = fkar * horvav / (log(zf(k+1) / z0m(i,j)) - psim(zf(k+1) / obl(i,j)) + psim(z0m(i,j) / obl(i,j)))
              endif
            end if
          else
            if(lhetero) then
          ...
          phimzf = phim(zf(k+1)/obl(i,j)) !cstep IBM
          phihzf = phih(zf(k+1)/obl(i,j)) !cstep IBM
          ...
          tskin(i,j) = min(max(thlflux(i,j) / (Cs(i,j) * horv),-10.),10.)  + thl0(i,j,k+1) !cstep IBM
          qskin(i,j) = min(max( qtflux(i,j) / (Cs(i,j) * horv),-5e-2),5e-2) + qt0(i,j,k+1) !cstep IBM
      ...
      do i=2,i1
        do j=2,j1
          ...
          if(lhetero) then
            patchx = patchxnr(i)
            patchy = patchynr(j)
            Rib    = grav / thvs_patch(patchx,patchy) * zf(k+1) * (thv - thvsl) / horv2
          else
            Rib    = grav / thvs * zf(k+1) * (thv - thvsl) / horv2
          endif
          ...
          if (Rib == 0) then
             ...
             do while (.true.)
                ...
                Lstart  = L - 0.001*L
                Lend    = L + 0.001*L
                fxdif   = ( (- zf(k+1) / Lstart * (log(zf(k+1) / z0h(i,j)) - psih(zf(k+1) / Lstart) + psih(z0h(i,j) / Lstart)) /&
                     (log(zf(k+1) / z0m(i,j)) - psim(zf(k+1) / Lstart) + psim(z0m(i,j) / Lstart)) ** 2.) - (-zf(k+1) / Lend * &
                     (log(zf(k+1) / z0h(i,j)) - psih(zf(k+1) / Lend) + psih(z0h(i,j) / Lend)) / (log(zf(k+1) / z0m(i,j)) - psim(zf(k+1) / Lend)&
