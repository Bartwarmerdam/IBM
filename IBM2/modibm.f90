...
!libm    deallocate(limmersed_boundary)
  ...
  subroutine applyibm   !< apply immersed boundary method
...
!cstep not used          umg(i,j)  = um(i,j,1)  - shear(i,j,1,10)*dzf(1)/nu_a

          call wallaw(v0(i,j,1),0.5*dzf(1),nu_a,shear(i,j,1,12))
          v0g(i,j)  = v0(i,j,1)  - shear(i,j,1,12)*dzf(1)/nu_a
!cstep not used         vmg(i,j)  = -vm(i,j,1) - shear(i,j,1,12)*dzf(1)/nu_a
